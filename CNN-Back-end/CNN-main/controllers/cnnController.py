...
models = ["./model/a.keras","./model/b.keras","./model/a.keras"]
...
def predict(path):
    ...
    saved_model = tf.keras.models.load_model(model_path) #carrega o modelo
    prediction = saved_model.predict(img_array)
    pred_index = np.argmax(prediction)
    return classes[pred_index] #retorna o nome da doença
    ...
    if request.method == 'POST':
        try:
            ...
            return {
                ...
                'prediction': results, #isso tem que retornar as %
                'glioma': percentg,
                'pituitary': percentp,
                'meningioma': percentm,
                'notumor': percentn,
